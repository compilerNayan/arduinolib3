...
This script uses extract_id_fields to find _Id fields in a class,
...
def process_file(file_path: str, serializable_macro: str = "_Entity", dry_run: bool = False) -> bool:
    """
    Process a file: extract _Id fields and inject primary key methods.
    ...
    if not id_fields:
        print(f"No _Id fields found in {result.get('class_name')}, skipping")
        return False
    
    # Use the first _Id field as the primary key
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Inject GetPrimaryKey() methods into classes with _Id fields"
    )
    parser.add_argument(
        "file_path",
