...
try:
    ...
    debug_print(f"Warning: Could not import extract_id_fields: {e}")
...
def inject_primary_key_methods(file_path: str, class_name: str, field_type: str, field_name: str, dry_run: bool = False) -> bool:
    ...
    if 'GetPrimaryKey()' in class_content or 'GetTableName()' in class_content:
        debug_print(f"⚠️  Primary key methods already exist in {class_name}, skipping injection")
        return False
    ...
    if dry_run:
        debug_print(f"Would inject the following methods into {class_name} in {file_path}:")
        debug_print("=" * 60)
    ...
    try:
        ...
        debug_print(f"✓ Injected GetPrimaryKey() methods into {class_name} in {file_path}")
        return True
    except Exception as e:
...
def process_file(file_path: str, serializable_macro: str = "_Entity", dry_run: bool = False) -> bool:
    ...
    if not HAS_EXTRACT_ID:
        debug_print("Error: extract_id_fields module not available")
        return False
    ...
    if not result or not result.get('has_serializable'):
        ...
        if serializable_macro == "_Entity":
            ...
            annotation_name = "@Serializable"
        debug_print(f"File {file_path} does not have {annotation_name} annotation, skipping")
        return False
    
    id_fields = result.get('id_fields', [])
    
    if not id_fields:
        debug_print(f"No @Id fields found in {result.get('class_name')}, skipping")
        return False
    ...
    debug_print(f"Found primary key field in {class_name}: {field_type} {field_name}")
